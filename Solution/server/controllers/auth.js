...
export const login = async (req, res) => {
  try {
    ...
    return res.status(200).json({ result: { email: user.email, token }, message: "You successfully logged in" });
  } catch (error) {
    console.log(error);
    ...
    registerValidationSchema
      ...
      .then(async (validationData) => {
        if (await User.countDocuments({ email: validationData.email }))
          return res.status(400).json({ error: `Email ${validationData.email} is already taken` });

        const hashedPassword = await bcrypt.hash(validationData.password, 12);
        const data = await User.create({ email: validationData.email, password: hashedPassword });
        ...
        return res
          .status(201)
          .json({ result: { _id: data._id, username: data.username }, token, message: "You successfully signed up" });
      })
